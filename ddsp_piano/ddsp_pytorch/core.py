...
# compute mean and std of loudness of dataset? 
@torch.no_grad()
...
# get a list of multiscale of the spectrogram
...
def remove_above_nyquist(amplitudes, pitch, sample_rate):
...
def crop_and_compensate_delay(audio, 
                              audio_size, 
                              ir_size,
                              padding,
                              delay_compensation):
...
def frequency_filter(
    ...
    mel_basis = None):
    """
    Filter audio with a finite impulse reponse filter
    Args:
        audio: Input audio. Tensor of shape [batch, audio_timesteps].
        magnitudes: Frequency transfer curve. 
        Float32 Tensor of shape [batch, n_frames, n_frequencies].
        The frequencies of the last dimension are ordered as [0, f_nyquist/(n_frequencies-1), 
        .... , f_nyquist], where f_nyquist is (sample_rate / 2). Automatically splits the audio 
        into equally sized frames to math frames in magnitudes.
        window_size: Size of the window to apply in the time domain. If window_size is less than 1,
        it is set as the default (n_frequencies).
        padding: Either 'valid', 'same'. For 'same' the final output to be the same size as
        the input audio (audio_timesteps). For 'valid' the audio is extended to include the
        tail of the impusle response (audio_timesteps + window_size - 1)
    Returns:
        Filtered audio. Tensor of shape 
        [batch, audio_timesteps + window_size - 1] ('valid' padding)
        [batch, audio_timesteps] ('same' padding)
    """
